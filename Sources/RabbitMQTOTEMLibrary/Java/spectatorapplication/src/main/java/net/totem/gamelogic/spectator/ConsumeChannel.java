...
public class ConsumeChannel extends RabbitMQGameInstanceChannel {
	...
	private List<List<Map<String, ? extends SpectatorActionInterface>>> listsOfListOfActions;
	private boolean rawActionsEnabled;

	ConsumeChannel(	final ChannelsManager channelsManager,
			final SpectatorState state,
			final List<Map<String, ? extends SpectatorActionInterface>> loai,
			boolean enableRawActions) 
			throws IOException {
		super(channelsManager, state);
		listsOfListOfActions = new Vector<List<Map<String, ? extends SpectatorActionInterface>>>();
	...
	private void startConsumeLoopThread(final SpectatorState state){
		new Thread() {
			public void run() {
				consumeLoop(state);
			}
	...
	private void consumeLoop(final SpectatorState state) {
		if (state == null) {
		...
		while (!state.hasConnectionExited()) {
			try {
				...
				// Error on acknowledgment of message: message will be inserted at the end of the Spectator queue
				consumer = newQueueingConsumer(e, state);
				if(consumer != null){
					// directly try to get a new delivery
					continue;
	...
	private QueueingConsumer getQueingConsumer(final SpectatorState state){
	...
	private void handleDelivery(QueueingConsumer.Delivery delivery, final SpectatorState state){
		if ((delivery != null) && (delivery.getBody() != null) && (delivery.getEnvelope().getRoutingKey() != null)) {
			...
			Util.println(" [Spectator " + state.login + "] received on "
			...
			if (header.length == 4) {
				...
				try {
					...
					Util.println(" [Spectator " + state.login
							+ "] Unknown action: " + e.getMessage());
					e.printStackTrace();
				}
			}else{
				Util.println(" [Spectator " + state.login + "] Malformed message "
						+ "(header of length " + header.length
						+ "instead of 4)");
				Util.println(" [Spectator " + state.login + "] Ignore message");
			}
		}
	}
	...
	private Object executeAction(final String nameKind,
			final String nameAction, final SpectatorState state,
			final String[] header, final String body)
	throws ActionInvocationException {
		Object result = null;
		boolean found = false;
		for (List<Map<String, ? extends SpectatorActionInterface>> aml : listsOfListOfActions) {
			for (Map<String, ? extends SpectatorActionInterface> am : aml) {
				SpectatorActionInterface action = am.get(nameKind
						+ Util.getRabbitMQProperties().getProperty(
	...
	private Object executeRawAction(final SpectatorState state, 
			final String[] header, final String body) 
	throws ActionInvocationException{
		Object result = null;
		boolean found = false;
		for (List<Map<String, ? extends SpectatorActionInterface>> aml : listsOfListOfActions) {
			for (Map<String, ? extends SpectatorActionInterface> am : aml) {
				SpectatorActionInterface action = am.get(RAW_ACTION_KIND
						+ Util.getRabbitMQProperties().getProperty(
		...
		if (!found) {
			throw new ActionInvocationException("WARNING : Raw action kind is not registered in the list of Spectator Actions. "+
	 ...
	 */
	private QueueingConsumer newQueueingConsumer(Exception e, final SpectatorState state){
		QueueingConsumer consumer = null;
		if(!state.hasConnectionExited()){
			Util.println(" [Spectator " + state.login + "] "+e.getClass().getSimpleName()+" : "+ e.getMessage());
			e.printStackTrace();
			if (channel.isOpen()) {
				// close channel and connection
				close();
