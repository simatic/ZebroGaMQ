...
class RabbitMQGameInstanceChannel {
	...
	RabbitMQGameInstanceChannel(final ChannelsManager channelsManager, final SpectatorState state) throws java.io.IOException {
		this.channelsManager = channelsManager;
		initCommunicationWithBroker(state);
	}
	

	protected boolean initCommunicationWithBroker(final SpectatorState state){
	...
	private void handleRetryNumber(int retryNumber, int maxRetryNumber, final SpectatorState state){
		// update state's attribute
		updateNumberOfRetries(retryNumber, state);
		if (retryNumber == maxRetryNumber - 1){
			Util.println(" [Spectator " + state.login+ "]: "+getClass().getSimpleName()+" stop : too many retries.");
	...
	private void updateNumberOfRetries(int retryNumber, final SpectatorState state){
		synchronized (state) {
