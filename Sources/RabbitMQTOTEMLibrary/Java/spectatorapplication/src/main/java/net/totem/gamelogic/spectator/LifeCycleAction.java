...
public enum LifeCycleAction implements SpectatorActionInterface {
	INITIATE_TERMINATTION("terminate") {
		public Object execute(final SpectatorState state, final String[] header,
				final String body) throws ActionInvocationException {
			return SpectatorProtocol.terminate(state, header,
					body);
		}
	};
	
	private final static Map<String, LifeCycleAction> privateActionMap = new HashMap<String, LifeCycleAction>();
	public final static Map<String, LifeCycleAction> actionMap = Collections.unmodifiableMap(privateActionMap);
