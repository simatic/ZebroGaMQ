...
public class ChannelsManager {
	 ...
	 * 
	 * @param state the SpectatorState of the player
	 * @param loai	the list of SpectatorAction 
	 ...
	 */
	public static ChannelsManager getInstance(	final SpectatorState state,
	 ...
	 * Instantiate the ChannelsManager, required for the consumption and the publication of messages.
	 * As soon as this method is called, the consumption of messages is automatically started.
	 * If the action kind and the action name of the messages consumed are not registered in the list of SpectatorAction,
	...
	public static ChannelsManager getInstance(	final SpectatorState state,
												...
												throws IOException{
		return new ChannelsManager(state, loai, enableRawAction);
	}
	
	
	private ChannelsManager(final SpectatorState state,
							final List<Map<String, ? extends SpectatorActionInterface>> loai,
							boolean enableRawAction) 
							throws IOException {
		// initialize state
		initSpectatorState(state);
	...
	private void initSpectatorState(SpectatorState state){
	...
	private ConnectionFactory getConnectionFactory(final SpectatorState state){
		...
		// set the heartbeat value for the amqp connection
	...
	public void exit() {
		Util.println(" [Spectator] exiting: closing channels and connections");
	 ...
	 * @param 	state		the SpectatorState of the spectator
	 ...
	 * 			The method SpectatorState.exit() should be called on the handling of this exception.
	 */
	public void publish(final String consumer, final SpectatorState state,
	 ...
	 * 
	 * @param 	consumer 	the login of the recipient
	 * @param 	state		the PlayerState of the player
	...
	public void publish(final String consumer, final SpectatorState state,
						...
						throws IOException, MissedHeartbeatException {
		publishChannel.publish(consumer, state, action, message);
	}
	 ...
	 * 			The method SpectatorState.exit() should be called on the handling of this exception.
	 */
	public void publishToAll(	final SpectatorState state,
								final SpectatorActionInterface action, final String message)
								throws IOException {
	 ...
	 * 			The method SpectatorState.exit() should be called on the handling of this exception.
	 */
	public void publishToGameLogicServer(	final SpectatorState state,
											final SpectatorActionInterface action, final String message)
											throws IOException {
