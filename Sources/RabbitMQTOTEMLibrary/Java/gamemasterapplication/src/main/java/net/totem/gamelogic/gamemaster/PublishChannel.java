...
public class PublishChannel extends RabbitMQGameInstanceChannel {
	...
	PublishChannel(final ChannelsManager channelsManager, final GameMasterState state) throws IOException {
	...
	void publish(final String consumer, final GameMasterState state,
		...
		while(communicationOK) {
			try {
				channel.basicPublish(state.exchangeName, routingKey, null,
						message.getBytes());
				Util.println(" [Master " + state.login + "] Sent " + message
	...
	void publishToAll(final GameMasterState state,
			final GameMasterActionInterface action, final String message)
	throws IOException {
		if (state == null) {
			throw new IllegalArgumentException("publish tries to publish"
	...
	void publishToGameLogicServer(final GameMasterState state,
			final GameMasterActionInterface action, final String message)
	throws IOException {
		if (state == null) {
			throw new IllegalArgumentException("publish tries to publish"
