...
public class ConsumeChannel extends RabbitMQGameInstanceChannel {
	...
	private List<List<Map<String, ? extends PlayerActionInterface>>> listsOfListOfActions;
	private boolean rawActionsEnabled;

	ConsumeChannel(	final ChannelsManager channelsManager,
			final PlayerState state,
			final List<Map<String, ? extends PlayerActionInterface>> loai,
			boolean enableRawActions) 
			throws IOException {
		super(channelsManager, state);
		listsOfListOfActions = new Vector<List<Map<String, ? extends PlayerActionInterface>>>();
	...
	private void startConsumeLoopThread(final PlayerState state){
		new Thread() {
			public void run() {
				consumeLoop(state);
			}
	...
	private void consumeLoop(final PlayerState state) {
		if (state == null) {
		...
		while (!state.hasConnectionExited()) {
			try {
				...
				// Error on acknowledgment of message: message will be inserted at the end of the player queue
				consumer = newQueueingConsumer(e, state);
				if(consumer != null){
					// directly try to get a new delivery
					continue;
	...
	private QueueingConsumer getQueingConsumer(final PlayerState state){
	...
	private void handleDelivery(QueueingConsumer.Delivery delivery, final PlayerState state){
		if ((delivery != null) && (delivery.getBody() != null) && (delivery.getEnvelope().getRoutingKey() != null)) {
			...
			Util.println(" [Player " + state.login + "] received on "
			...
			if (header.length == 4) {
				...
				Util.println(" [Player " + state.login + "] Malformed message "
						+ "(header of length " + header.length
						+ "instead of 4)");
				Util.println(" [Player " + state.login + "] Ignore message");
			}
		}
	}
	...
	private Object executeAction(final String nameKind,
			final String nameAction, final PlayerState state,
			final String[] header, final String body)
	throws ActionInvocationException {
		Object result = null;
		boolean found = false;
		for (List<Map<String, ? extends PlayerActionInterface>> aml : listsOfListOfActions) {
			for (Map<String, ? extends PlayerActionInterface> am : aml) {
				PlayerActionInterface action = am.get(nameKind
						+ Util.getRabbitMQProperties().getProperty(
	...
	private Object executeRawAction(final PlayerState state, 
									final String[] header, final String body) 
	throws ActionInvocationException{
		Object result = null;
		boolean found = false;
		for (List<Map<String, ? extends PlayerActionInterface>> aml : listsOfListOfActions) {
			for (Map<String, ? extends PlayerActionInterface> am : aml) {
				PlayerActionInterface action = am.get(RAW_ACTION_KIND
						+ Util.getRabbitMQProperties().getProperty(
		...
		if (!found) {
			throw new ActionInvocationException("WARNING : Raw action kind is not registered in the list of Player Actions. "+
	 ...
	 */
	private QueueingConsumer newQueueingConsumer(Exception e, final PlayerState state){
		QueueingConsumer consumer = null;
		if(!state.hasConnectionExited()){
			Util.println(" [Player " + state.login + "] "+e.getClass().getSimpleName()+" : "+ e.getMessage());
			e.printStackTrace();
			//if (channel.isOpen()) {
				// close channel and connection
				close();
