...
public class PublishChannel extends RabbitMQGameInstanceChannel {
	...
	PublishChannel(final ChannelsManager channelsManager, final PlayerState state) throws IOException {
	...
	void publish(final String consumer, final PlayerState state,
		...
		while(communicationOK) {
			try {
				channel.basicPublish(state.exchangeName, routingKey, null,
						message.getBytes());
				Util.println(" [Player " + state.login + "] Sent " + message
	...
	void publishToAll(final PlayerState state,
			final PlayerActionInterface action, final String message)
	throws IOException {
		if (state == null) {
			throw new IllegalArgumentException("publish tries to publish"
	...
	void publishToGameLogicServer(final PlayerState state,
			final PlayerActionInterface action, final String message)
	throws IOException, MissedHeartbeatException {
		if (state == null) {
			throw new IllegalArgumentException("publish tries to publish"
