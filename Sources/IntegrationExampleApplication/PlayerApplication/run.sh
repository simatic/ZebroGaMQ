...
CLASS=net.totem.integration.player.j2se.PlayerApplication
RABBITMQPROPERTIESFILE=rabbitmq.properties
XMLRPCPROPERTIESFILE=xmlrpc.properties

if [[ -f ./target/gameplayer-${MODULE_VERSION}.jar ]]
then
    export JARS=./target/gameplayer-${MODULE_VERSION}.jar
else
    echo Archive file ./target/gameplayer-${MODULE_VERSION}.jar missing
...
if [[ -f ${HOME}/.m2/repository/net/totem/gamelogic/gameplayer/0.1-SNAPSHOT/gameplayer-0.1-SNAPSHOT.jar ]]
then
    export JARS=${HOME}/.m2/repository/net/totem/gamelogic/gameplayer/0.1-SNAPSHOT/gameplayer-0.1-SNAPSHOT.jar:${JARS}
else
    echo Archive file ${HOME}/.m2/repository/net/totem/gamelogic/gameplayer/0.1-SNAPSHOT/gameplayer-0.1-SNAPSHOT.jar missing
    echo Run maven install to install it on your local maven repository
fi
...
exec java -Drabbitmq.config.file=${RABBITMQPROPERTIESFILE} -Dxmlrpc.config.file=${XMLRPCPROPERTIESFILE} -cp ${JARS} ${CLASS} $1 $2 $3 $4 &
