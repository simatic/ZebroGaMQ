...
import net.totem.gamelogic.gamemaster.ChannelsManager;
import net.totem.gamelogic.gamemaster.GameMasterState;
import net.totem.gamelogic.gamemaster.JoinAction;
import net.totem.gamelogic.gamemaster.PresenceAction;
...
public class MasterApplication {
	...
	private static GameMasterState state = null;
	...
	public static void main(final String[] argv) throws java.io.IOException,
		...
		if (argv.length != 4) {
			Util.println(" [Master] Please, only four strings"
					+ " are accepted "
	...
	private static Object executeXMLRPCCall() {
		...
		try {
			...
			answer = client.execute("createGameInstance", params);
	...
	private static boolean computeXMLRPCAnswer(Object answer) {
		...
		if (answer == null) {
			...
			Util.println("Game server created " + state.gameName + "/"
					+ state.gameInstanceName + " for master " + state.login);
			try {
			...
			Util.println("Game server did not create " + state.gameName + "/"
					+ state.gameInstanceName + " for master " + state.login);
			Util.println(" [Master " + state.login + "] Exiting ");
			return false;
		}
	}
