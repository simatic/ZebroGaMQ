...
CLASS=net.totem.integration.spectator.j2se.SpectatorApplication
RABBITMQPROPERTIESFILE=rabbitmq.properties
XMLRPCPROPERTIESFILE=xmlrpc.properties

if [[ -f ./target/gamespectator-${MODULE_VERSION}.jar ]]
then
    export JARS=./target/gamespectator-${MODULE_VERSION}.jar
else
    echo Archive file ./target/gamespectator-${MODULE_VERSION}.jar missing
...
if [[ -f ${HOME}/.m2/repository/net/totem/gamelogic/gamespectator/0.1-SNAPSHOT/gamespectator-0.1-SNAPSHOT.jar ]]
then
    export JARS=${HOME}/.m2/repository/net/totem/gamelogic/gamespectator/0.1-SNAPSHOT/gamespectator-0.1-SNAPSHOT.jar:${JARS}
else
    echo Archive file ${HOME}/.m2/repository/net/totem/gamelogic/gamespectator/0.1-SNAPSHOT/gamespectator-0.1-SNAPSHOT.jar missing
    echo Run maven install to install it on your local maven repository
fi
...
# argv[4] = observation key for the application instance
exec java -Drabbitmq.config.file=${RABBITMQPROPERTIESFILE} -Dxmlrpc.config.file=${XMLRPCPROPERTIESFILE} -cp ${JARS} ${CLASS} $1 $2 $3 $4 $5 &
