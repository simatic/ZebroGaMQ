...
public class PublishChannel extends RabbitMQGameInstanceChannel {
	...
	PublishChannel(final ChannelsManager channelsManager, final GameLogicState state) throws IOException {
	...
	void publish(final String consumer, final GameLogicState state,
		...
		while(communicationOK) {
			try {
				channel.basicPublish(state.exchangeName, routingKey, null,
						message.getBytes());
				Util.println(" ["+state.role+" " + state.login + "] Sent " + message
	...
	void publishToAll(final GameLogicState state,
			final GameLogicActionInterface action, final String message)
	throws IOException {
		if (state == null) {
			throw new IllegalArgumentException("publish tries to publish"
	...
	void publishToGameLogicServer(final GameLogicState state,
			final GameLogicActionInterface action, final String message)
	throws IOException {
		if (state == null) {
			throw new IllegalArgumentException("publish tries to publish"
