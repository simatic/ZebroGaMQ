...
IF EXIST "%PWD%target\zebrogamq-gamelogic-perf-application-%MODULE_VERSION%.jar" (
	set JARS="%PWD%target\zebrogamq-gamelogic-perf-application-%MODULE_VERSION%.jar";%JARS%
) ELSE (
...
rem argv[5] = nb msgs
rem argv[6] = millisec between msg
rem argv[7] = payload size

start java -Drabbitmq.config.file=%RABBITMQPROPERTIESFILE% -Dxmlrpc.config.file=%XMLRPCPROPERTIESFILE% -cp %JARS% %CLASS% %1 %2 %3 %4 %5 %6 %7 %8
